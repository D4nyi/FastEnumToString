...
{{
    ...
    {{{3}    }}
        ...
        public readonly int _fallback;

        private readonly HashSet<string> _namespaces = new HashSet<string>();
        public static string Empty => $"//<auto-generated />\r\n\r\n// Code will be generated here, yet we found no enums.";


        public EnumProcessor(GeneratorExecutionContext context, string rootNamespace, int fallback)
        {
            _context = context;
            _rootNamespace = rootNamespace;
            _fallback = fallback < -1 ? -1 : fallback;
        }
        ...
        private string ProcessEnum(IReadOnlyCollection<EnumDeclarationSyntax> enums)
        {
            ...
            foreach (EnumDeclarationSyntax @enum in enums)
            {
                ...
                AttributeSyntax overrideAttr = @enum.AttributeLists.GetOverrideAttribute();
                bool hasOverride = !(overrideAttr is null);
                int overridenFallback = -2;
                if (hasOverride)
                {
                    SemanticModel semanticModel = _context.Compilation.GetSemanticModel(overrideAttr.SyntaxTree);
                    if (!(overrideAttr.ArgumentList is null) && overrideAttr.ArgumentList.Arguments.Any())
                    {
                        AttributeArgumentSyntax overrideArg = overrideAttr.ArgumentList.Arguments[0];
                        ExpressionSyntax overrideExpr = overrideArg.Expression;
                        Optional<object> routeTemplate = semanticModel.GetConstantValue(overrideExpr, _context.CancellationToken);
                        if (routeTemplate.HasValue && !Int32.TryParse(routeTemplate.Value.ToString(), out overridenFallback))
                        {
                            overridenFallback = -1;
                        }
                    }
                ...
                string body = CreateBody(@enum.Members, symbol.Name, hasOverride, overridenFallback);
        ...
        private string CreateBody(SeparatedSyntaxList<EnumMemberDeclarationSyntax> enumMembers, string enumName, bool overrideFallback, int overridenFallback)
        {
            StringBuilder builder = new StringBuilder();

            // it's read once instead of in every iteration
            int count = enumMembers.Count;
            int fallback = overrideFallback
                ? CheckFallbackBoundary(overridenFallback, count)
                : CheckFallbackBoundary(_fallback, count);
            ...
            if (fallback == -1 || count == 0)
            {
                builder.Append(SwitchDefaultThrow);
            }
            else
            {
                builder.AppendFormat(SwitchDefault, enumName, enumMembers[fallback].Identifier.Text);
            }

            return builder.ToString();
        }

        private static int CheckFallbackBoundary(int fallback, int count)
        {
            return (fallback >= count || fallback < -1) ? -1 : fallback;
        }
    }
}
